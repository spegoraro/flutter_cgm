...
class PolyBezier extends Command {
  ...
  late final List<Point<double>> p1;
  late final List<Point<double>> p2;
  late final List<Point<double>> p3;
  late final List<Point<double>> p4;
  ...
  PolyBezier(super.ec, super.eid, super.l, super.buffer, super.cgm) {
    ...
    if (_continuityIndicator == 1) {
      ...
      p1 = List.filled(n, const Point(0, 0));
      p2 = List.filled(n, const Point(0, 0));
      p3 = List.filled(n, const Point(0, 0));
      p4 = List.filled(n, const Point(0, 0));
  ...
  void _initPath() {
    ...
    for (final p in paths) {
      path!.extendWithPath(p, Offset.zero);
    }
  }
  ...
  void paint(CGMDisplay display, [BeginFigure? beginFigure]) {
    ...
    var random = Random(getSeed().hashCode);
    randomColor() => Color.fromARGB(255, random.nextInt(255), random.nextInt(255), random.nextInt(255));
    final canvas = display.canvas;
    if (path == null) _initPath();

    if (mode == 0) {
      canvas.drawPath(path!, display.linePaint!);
    } else {
      var paint = display.linePaint!;
      if (display.currentPaintHolder?.edgeColor != null || display.currentPaintHolder?.edgeWidth != null) {
        paint = display.edgePaint!;
      }

      if (display.currentPaintHolder?.fillColor != null) {
